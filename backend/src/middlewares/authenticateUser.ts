...
const authenticateUser = async (req: Request, res: Response, next: NextFunction) => {
  const {
    body: { user, password, type },
  } = req;
  try {
    if (!type) return res.status(400).json(errorResponse('Insira o "type" da conta no body'));

    const registeredUser = await readOneByUser(type, user);
    ...
    res.cookie('data', { user, type, token });
